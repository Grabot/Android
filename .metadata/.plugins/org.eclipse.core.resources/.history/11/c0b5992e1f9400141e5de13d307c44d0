...
public class TownScreen implements Screen
{
	...
	public TownScreen( Application app, OrthographicCamera camera, Stage stage, SpriteBatch batch, ArrayList<Citizen> citizens )
	{
		this.citizens = citizens;
		Gdx.app.log( LOG, "Creating opening screen" );
		
		simulation = new Simulation( this, citizens );
		renderer = new Renderer( this, simulation, camera, stage, batch, citizens );
	...
	public void update(float delta, float touchX, float touchY, float width, float height, boolean touched_down, boolean fast_press, boolean back_pressed, boolean down_pressed, boolean enter_pressed, boolean up_pressed, float distance ) 
	{
		...
		simulation.variables( touchX, touchY, width, height, touched_down, fast_press, back_pressed, down_pressed, enter_pressed, up_pressed, distance );
	}
