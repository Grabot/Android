...
public class DrawTiles extends Data {
	...
	public void fillTiles(Simulation simulation, Batch batch) {
		...
		for (int x = 0; x < level.getWidth(); x++) {
			for (int y = 0; y < level.getHeight(); y++ ){
				batch.draw(SquareTileRegionGrass, x, y);
			}
		}
	...
	public void drawSelected(Simulation simulation, Batch batch, int tileNumber) {
		// batch.draw( SquareTileRegionSelected,
		// (-SquareTileSelected.getWidth()/2 +
		// simulation.tiles.get(tileNumber).getPosition().x),
		// (-SquareTileSelected.getHeight()/2 +
		// simulation.tiles.get(tileNumber).getPosition().y), 0, 0,
		// SquareTileSelected.getWidth(), SquareTileSelected.getHeight(), 1, 1,
		// 0, false);
	}
